...
void exibir_estado_filas(Guiche* guiches, int qtd_guiches,
                         string tipo_guiche_label) {
    ...
    for (int i = 0; i < qtd_guiches; i++) {
        cout << "Guiche " << (i + 1) << " (" << guiches[i].fila.tamanho
             << " na fila): ";
        ItemFila<Torcedor>* atual = guiches[i].fila.inicio;
        while (atual != nullptr) {
            cout << "[" << (atual->dado.tipo ? "S" : "N") << ","
                 << atual->dado.UnidadeTempo << "] ";
            atual = atual->prox;
        }
        cout << "\n";
    }
...
int main() {
    ...
    cout << "Quantidade de pessoas que procuram guiches a cada unidade de "
            "tempo: ";
...
*/
    */ Guiche* guichesSocios = nullptr;
    if (dados.qtd_guiche_socio > 0) {
        ...
        for (int i = 0; i < dados.qtd_guiche_socio;
             i++) {  // loop para iniciar todas as posições do vetor
            inicializa_Guiche(guichesSocios[i]);
        }
    }

    Guiche* guichesNormais = nullptr;
    if (dados.qtd_guiche_normal > 0) {  // mesma coisa que o trecho acima
    ...
    if (dados.carga_inicial > 0) {
        cout << "\nProcessando carga inicial de " << dados.carga_inicial
             << " torcedores...\n";
        distribuir_carga_turno(dados.carga_inicial, guichesSocios,
                               guichesNormais, dados);
    }

    exibir_estado_filas(guichesSocios, dados.qtd_guiche_socio,
                        "Socio-Torcedor");
    exibir_estado_filas(guichesNormais, dados.qtd_guiche_normal, "Normal");

    for (int tempo_atual = 1; tempo_atual <= dados.qtd_tempo_total;
         tempo_atual++) {
        if (tempo_atual == 1) {
            cin.ignore();
        }
        ...
        if (dados.carga_por_turno > 0) {
            cout << "Novas chegadas (" << dados.carga_por_turno
                 << " torcedores)...\n";
            distribuir_carga_turno(dados.carga_por_turno, guichesSocios,
                                   guichesNormais, dados);
        }
        ...
        float total_direcionados_geral =
            dados.total_socios_procuraram + dados.total_normais_procuraram;
        if (total_direcionados_geral > 0) {
            float perc_socios_direcionados =
                dados.total_socios_procuraram / total_direcionados_geral * 100;
            float perc_normais_direcionados =
                dados.total_normais_procuraram / total_direcionados_geral * 100;

            cout << "Proporcao de Torcedores Direcionados (Acumulada): Socios "
                 << perc_socios_direcionados << "% ("
                 << dados.total_socios_procuraram << "), "
                 << "Normais " << perc_normais_direcionados << "% ("
                 << dados.total_normais_procuraram << ")\n";
        } else {
        ...
        if (dados.qtd_guiche_socio > 0) {
            for (int i = 0; i < dados.qtd_guiche_socio; i++) {
                if (guichesSocios[i].fila.tamanho != 0) {
                    guiches_ocupados_socios++;
                }
            }
            float perc_ocup_socios =
                guiches_ocupados_socios / dados.qtd_guiche_socio * 100;
        ...
        if (dados.qtd_guiche_normal > 0) {
            for (int i = 0; i < dados.qtd_guiche_normal; i++) {
                if (guichesNormais[i].fila.tamanho != 0) {
                    guiches_ocupados_normais++;
                }
            }
            float perc_ocup_normais =
                guiches_ocupados_normais / dados.qtd_guiche_normal * 100;
        ...
        float total_socios_com_tempo_definido =
            dados.socios_atend_1_ut + dados.socios_atend_2_ut;
        if (total_socios_com_tempo_definido > 0) {
            cout << "  Socios:\n";
            imprimir_porcentagem(dados.socios_atend_1_ut,
                                 total_socios_com_tempo_definido,
                                 "    - 1 UT: ");
            cout << " | ";
            imprimir_porcentagem(dados.socios_atend_2_ut,
                                 total_socios_com_tempo_definido, "2 UT: ");
            cout << "\n";
        }
        float total_normais_com_tempo_definido = dados.normais_atend_1_ut +
                                                 dados.normais_atend_2_ut +
                                                 dados.normais_atend_3_ut;
        if (total_normais_com_tempo_definido > 0) {
            cout << "  Normais:\n";
            imprimir_porcentagem(dados.normais_atend_1_ut,
                                 total_normais_com_tempo_definido,
                                 "    - 1 UT: ");
            cout << " | ";
            imprimir_porcentagem(dados.normais_atend_2_ut,
                                 total_normais_com_tempo_definido, "2 UT: ");
            cout << " | ";
            imprimir_porcentagem(dados.normais_atend_3_ut,
                                 total_normais_com_tempo_definido, "3 UT: ");
            cout << "\n";
        }
    ...
    cout << "\nResumo Final da Distribuicao de Tempo de Atendimento (Total de "
            "Torcedores com Tempo Definido):\n";
    float final_total_socios_com_tempo =
        dados.socios_atend_1_ut + dados.socios_atend_2_ut;
    if (final_total_socios_com_tempo > 0) {
        cout << "  Socios (Total: " << final_total_socios_com_tempo << "):\n";
        imprimir_porcentagem(dados.socios_atend_1_ut,
                             final_total_socios_com_tempo, "    - 1 UT: ");
        cout << " | ";
        imprimir_porcentagem(dados.socios_atend_2_ut,
                             final_total_socios_com_tempo, "2 UT: ");
        cout << "\n";
    } else {
        cout << "  Nenhum socio com tempo de atendimento definido.\n";
    }

    float final_total_normais_com_tempo = dados.normais_atend_1_ut +
                                          dados.normais_atend_2_ut +
                                          dados.normais_atend_3_ut;
    if (final_total_normais_com_tempo > 0) {
        cout << "  Normais (Total: " << final_total_normais_com_tempo << "):\n";
        imprimir_porcentagem(dados.normais_atend_1_ut,
                             final_total_normais_com_tempo, "    - 1 UT: ");
        cout << " | ";
        imprimir_porcentagem(dados.normais_atend_2_ut,
                             final_total_normais_com_tempo, "2 UT: ");
        cout << " | ";
        imprimir_porcentagem(dados.normais_atend_3_ut,
                             final_total_normais_com_tempo, "3 UT: ");
        cout << "\n";
    } else {
    ...
    if (guichesSocios) {
        for (int i = 0; i < dados.qtd_guiche_socio; i++) {
            total_torcedores_restantes_nas_filas +=
                guichesSocios[i].fila.tamanho;
        }
    ...
    if (guichesNormais) {
        for (int i = 0; i < dados.qtd_guiche_normal; i++) {
            total_torcedores_restantes_nas_filas +=
                guichesNormais[i].fila.tamanho;
        }
        total_guiches_ativos += dados.qtd_guiche_normal;
    }

    cout << "\nTorcedores restantes nas filas: "
         << total_torcedores_restantes_nas_filas << endl;

    if (total_guiches_ativos > 0) {
        float media_torcedores_por_fila =
            total_torcedores_restantes_nas_filas / total_guiches_ativos;
        cout << "Media de torcedores por fila ao final: "
             << media_torcedores_por_fila << endl;
    } else {
