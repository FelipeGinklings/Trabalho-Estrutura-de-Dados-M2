...
*/
    ...
    if (dados.qtd_guiche_socio > 0){ 
        guichesSocios = new Guiche[dados.qtd_guiche_socio]; // cria um vetor com o tamanho que o usuário escolheu 
        for (int i = 0; i < dados.qtd_guiche_socio; i++){ // loop para iniciar todas as posições do vetor
    ...
    for (int tempo_atual = 1; tempo_atual <= dados.qtd_tempo_total; tempo_atual++){
        ...
        float total_direcionados_geral = dados.total_socios_procuraram + dados.total_normais_procuraram;
        if (total_direcionados_geral > 0){
            float perc_socios_direcionados = dados.total_socios_procuraram / total_direcionados_geral * 100;  
            float perc_normais_direcionados = dados.total_normais_procuraram / total_direcionados_geral * 100;
        ...
        if (dados.qtd_guiche_socio > 0){
            ...
            float perc_ocup_socios = guiches_ocupados_socios / dados.qtd_guiche_socio * 100;
            cout << "Ocupacao Guiches Socios: " << perc_ocup_socios << "% (" << guiches_ocupados_socios << "/" << dados.qtd_guiche_socio << ")\n";
        }
        ...
        if (dados.qtd_guiche_normal > 0){
            ...
            float perc_ocup_normais = guiches_ocupados_normais / dados.qtd_guiche_normal * 100;
