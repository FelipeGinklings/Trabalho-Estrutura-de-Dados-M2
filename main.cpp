 ...
 *
 * Esta função aplica regras específicas de distribuição para calcular quantos itens
 * devem ser alocados com base em uma porcentagem dada. Ela trata vários casos específicos
 * com lógica especial antes de recorrer ao cálculo padrão por porcentagem.
 *
 ...
 * - Caso especial de 30% diminui do total 45% mais 1 e 25% para pegar o valor mais próximo de 25%
 * - Caso padrão adiciona 1 ao cálculo inicial de porcentagem
 *
 * @warning A função assume que porcentagem está no intervalo 0-100. Valores fora desta faixa
 *          podem produzir resultados inesperados.
 * @warning Esses cálculos são considerando que as porcentagens são 5 ou 15 ou 25 ou 30 ou 45 ou 85 ou 95
 */
...
int main() {
    for (int i = 1; i <= 100; i += 1) {
        const auto valor1 = quantidadePorPorcentagem(i, PORCENTAGEM_NORMAL_TEMPO_1);
        const auto valor2 = quantidadePorPorcentagem(i, PORCENTAGEM_NORMAL_TEMPO_2);
        const auto valor3 = quantidadePorPorcentagem(i, PORCENTAGEM_NORMAL_TEMPO_3);
        std::cout << valor1 << " + " << valor2 << " + " << valor3 << " = " << i << " esta soma "
                  << (valor1 + valor2 + valor3 == i ? "esta" : "nao esta") << " correta" << std::endl;
    }
    return 0;
}
